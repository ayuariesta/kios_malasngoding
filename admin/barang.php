...
<?php 
$periksa=mysqli_query($db, "select * from barang where jumlah <=3");
...
<?php 
$per_hal=20;
$jumlah_record=mysqli_query($db, "SELECT COUNT(*) from barang");
...
<table class="table table-hover">
	...
	if(isset($_GET['cari'])){
		...
		$brg=mysqli_query($db, "select * from barang where nama like '$cari' or jenis like '$cari'");
	}else{
		$brg=mysqli_query($db, "select * from barang limit $start, $per_hal");
	}
	$no=1;
	...
	<tr>
		...
		<?php 
			...
			$x=mysqli_query($db, "select sum(modal) as total from barang");	
