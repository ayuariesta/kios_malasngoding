...
<form action="" method="get">
	<div class="input-group col-md-5 col-md-offset-7">
		...
		<select type="submit" name="tanggal" class="form-control" onchange="this.form.submit()">
			...
			<?php 
			$pil=mysqli_query($db, "select distinct tanggal from barang_laku order by tanggal desc");
...
<table class="table">
	...
	if(isset($_GET['tanggal'])){
		$tanggal=mysqli_real_escape_string($db, $_GET['tanggal']);
		$brg=mysqli_query($db, "select * from barang_laku where tanggal like '$tanggal' order by tanggal desc");
	}else{
		$brg=mysqli_query($db, "select * from barang_laku order by tanggal desc");
	}
	$no=1;
	...
	<tr>
		...
		if(isset($_GET['tanggal'])){
			$tanggal=mysqli_real_escape_string($db, $_GET['tanggal']);
			$x=mysqli_query($db, "select sum(total_harga) as total from barang_laku where tanggal='$tanggal'");	
	...
	<tr>
		...
		if(isset($_GET['tanggal'])){
			$tanggal=mysqli_real_escape_string($db, $_GET['tanggal']);
			$x=mysqli_query($db, "select sum(laba) as total from barang_laku where tanggal='$tanggal'");	
			$xx=mysqli_fetch_array($x);			
			echo "<td><b> Rp.". number_format($xx['total']).",-</b></td>";
		}else{
					...
					<form action="barang_laku_act.php" method="post">
						...
						<div class="form-group">
							...
							<select class="form-control" name="nama">
								<?php 
								$brg=mysqli_query($db, "select * from barang");
								while($b=mysqli_fetch_array($brg)){
									?>	
